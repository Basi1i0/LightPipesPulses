...
for d in dirs[5:] :
    Izt0yx,s,lp,cp,g = SaveLoad.ReadDumpFromDisk('C://Users//Basil//ResearchData//FourierOptics//GaussianTF//' + d, 
                                                 namedtuplesGaussTF.LaunchParams, namedtuplesGaussTF.ComputParmas)
...
#    plt.ylim( 0, max(wt_z)*1.1/3e-7 ) 
    ...
    p = plt.plot(g.z.points/mm, Iyz2p[lp.Nx//2][lp.Nx//2] , 'o' )
    plt.plot(g.z.points/mm, 1/numpy.sqrt(r1(g.z.points, cp)*r2(g.z.points, cp)) /max(1/numpy.sqrt(r1(g.z.points, cp)*r2(g.z.points, cp))) * max(Iyz2p[lp.Nx//2][lp.Nx//2] ),
             color = p[0].get_color() )
...
plt.plot(ms[:,0], g.z.points[list(map(int, ms[:,1]))], 'o')
plt.plot(ms[:,0], lp.f * ms[:,3])
plt.show()
